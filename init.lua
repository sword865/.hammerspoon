...
hs.hotkey.bind({"ctrl","alt","cmd"}, "Left", function()
    sizeup.send_window_left_3()
  end)
  -- Send Window Right
  hs.hotkey.bind({"ctrl","alt","cmd"}, "Right", function()
    sizeup.send_window_right_3()
  end)
  -- Send Window Up
  hs.hotkey.bind({"ctrl","alt","cmd"}, "Up", function()
    sizeup.send_window_up_3()
  end)
  -- Send Window Down
  hs.hotkey.bind({"ctrl","alt","cmd"}, "Down", function()
    sizeup.send_window_down_3()
  end)
...
hs.hotkey.bind({"alt","ctrl"}, "Down", function()
   ...
   sizeup.move_to_center_relative({w=0.6, h=0.7})
end)
...
-- Partitions 3--
sizeup._split3_screen_partitions = {
    flip = true,
    p1 = {
        x = 0.34, -- %
        y = 0.66
...
function sizeup.split3_screen_partitions()
    sizeup._split3_screen_partitions.flip = not sizeup._split3_screen_partitions.flip 
    if sizeup._split3_screen_partitions.flip then
        return sizeup._split3_screen_partitions.p1
    else
        return sizeup._split3_screen_partitions.p2
    end
end
...
function sizeup.send_window_left_3()
    local s = sizeup.screen()
    local ssp = sizeup.split3_screen_partitions()
    local g = sizeup.gutter()
    sizeup.set_frame("Left 3", {
      x = s.x,
      y = s.y,
      w = (s.w * ssp.x) - sizeup.gutter().x,
      h = s.h
  ...
  function sizeup.send_window_right_3()
    local s = sizeup.screen()
    local ssp = sizeup.split3_screen_partitions()
    local g = sizeup.gutter()
    sizeup.set_frame("Right 3", {
      x = s.x + (s.w * ssp.x) + g.x,
      y = s.y,
      w = (s.w * (1 - ssp.x)) - g.x,
      h = s.h
  ...
  function sizeup.send_window_up_3()
    local s = sizeup.screen()
    local ssp = sizeup.split3_screen_partitions()
    local g = sizeup.gutter()
    sizeup.set_frame("Up 3", {
      x = s.x,
      y = s.y,
      w = s.w,
      h = (s.h * ssp.y) - g.y
  ...
  function sizeup.send_window_down_3()
    local s = sizeup.screen()
    local ssp = sizeup.split3_screen_partitions()
    local g = sizeup.gutter()
    sizeup.set_frame("Down 3", {
      x = s.x,
      y = s.y + (s.h * ssp.y) + g.y,
      w = s.w,
      h = (s.h * (1 - ssp.y)) - g.y
